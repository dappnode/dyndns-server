...
app.get("/", async function (req, res) {
  ...
  try {
    ...
    res
      .status(400)
      .send(JSON.stringify({ message: "Signing error: " + err.message }));
    return;
  }

  // Check if provided timestamp is in sync with us.
  const validTimestamp =
    epoch <= timestamp + threshold && epoch >= timestamp - threshold;

  if (signAddress.toLowerCase() !== address.toLowerCase()) {
    res
      .status(400)
      .send(JSON.stringify({ message: "Invalid address or signature." }));
  } else if (!validTimestamp) {
    res.status(400).send(
      JSON.stringify({
        message: "Timestamp out of sync. Is your server syncronized?"
      })
    );
  } else {
    ...
    try {
      ...
      res
        .status(500)
        .send(JSON.stringify({ message: "Internal error on update." }));
